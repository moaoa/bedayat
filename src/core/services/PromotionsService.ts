...
import {
  AddUpdatePromotionDto,
  Promotion,
  PromotionDto,
} from "@/types/Promotions";

class PromotionsService {
  public static async loadPromotions(
    searchValue: string
  ): Promise<PromotionDto[]> {
    const result = await ApiService.get<ApiResponse<PromotionDto[]>>(
      `${AppConstants.PROMOTIONS_URL}`,
      `?info=${searchValue}`
    );
  ...
  public static async updatePromotion(
    promotionId: string,
    data: AddUpdatePromotionDto
  ): Promise<ApiResponse<PromotionDto>> {
    const formData = new FormData();
  ...
  public static async activatePromotion(
    promotionId: string,
    state: string
  ): Promise<ApiResponse<PromotionDto>> {
    const result = await ApiService.put<ApiResponse<PromotionDto>>(
  ...
  public static async deletePromotion(
    promotionId: string
