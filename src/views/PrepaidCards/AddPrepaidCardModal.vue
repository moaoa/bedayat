    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      <div class="modal-content">
        <div class="modal-header" id="kt_modal_add_customer_header">
          <h2 class="fw-bolder">{{ $t("addNewPrepaidCard") }}</h2>
          <div
...
const formData = reactive<NewPrepaidCard>({
  coinAmount: 0,
  count: 0,
});

const rules = ref<Record<keyof NewPrepaidCard, object[]>>({
  coinAmount: [{ required: true, message: t("required"), trigger: "blur" }],
  count: [
    {
      required: true,
      message: t("required"),
...
const submit = () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      await prepaidCardsStore.createNewItem(formData);
